...
app.get('/api/supervisors/filter', async (req, res) => {
  try {
    ...
    if (searchQuery) {
      query += ` AND (full_name LIKE ? OR department LIKE ? OR position LIKE ? OR specialization LIKE ?)`;
    ...
    query += ` ORDER BY ${orderByField} ${sortDirection === 'desc' ? 'DESC' : 'ASC'}`;
...
app.get('/api/departments', async (req, res) => {
  try {
    const query = `
      SELECT DISTINCT department as department_id, department as department_name
      FROM supervisors
    ...
    console.error('Ошибка получения списка кафедр:', error);
...
app.get('/api/positions', async (req, res) => {
  try {
    const query = `
      SELECT DISTINCT position as position_id, position as position_name
      FROM supervisors
    ...
    console.error('Ошибка получения списка должностей:', error);
    res.status(500).json({ error: 'Ошибка сервера при получении данных' });
  }
});
...
app.get('/api/specializations', async (req, res) => {
  try {
    const query = `
      SELECT DISTINCT specialization as specialization_id, specialization as specialization_name
      FROM supervisors
    ...
    console.error('Ошибка получения списка специализаций:', error);
    res.status(500).json({ error: 'Ошибка сервера при получении данных' });
  }
});

// Получение списка статусов (обычно это фиксированный список значений)
app.get('/api/statuses', async (req, res) => {
  try {
    const query = `
      SELECT DISTINCT teacher_status as status_id, teacher_status as status_name
      FROM supervisors
...
// Получение количества студентов у каждого преподавателя
app.get('/api/supervisors/students-count', async (req, res) => {
  try {
    const query = `
      SELECT s.supervisor_id, COUNT(ss.student_id) as students_count
      FROM supervisors s
      LEFT JOIN supervisor_student ss ON s.supervisor_id = ss.supervisor_id
      GROUP BY s.supervisor_id
    `;
    const [counts] = await db.execute(query);
    res.json(counts);
  } catch (error) {
    console.error('Ошибка получения количества студентов:', error);
    res.status(500).json({ error: 'Ошибка сервера при получении данных' });
  }
});
...
app.get('/api/students/search', async (req, res) => {
  ...
  try {
    ...
    const sql = `
      ...
      WHERE CONCAT_WS(' ', 
        s.student_id, s.full_name, s.email, s.student_card_number, 
        s.department, s.study_group_id, s.phone, s.supervisor_id, 
        s.total_progress, s.diploma_id, g.group_name, g.specialty
      ) LIKE ?
    `;
    const queryParam = `%${q}%`;
    const [rows] = await db.query(sql, [queryParam]);
...
// Розширена фільтрація студентів з сортуванням
app.get('/api/students/filter', async (req, res) => {
  const { 
    q,                  // Пошуковий запит
    group_id,           // ID групи
    specialty_id,       // ID спеціальності
    department_id,      // ID департаменту/кафедри
    supervisor_id,      // ID наукового керівника
    progress_filter,    // Фільтр за прогресом (low, medium, high, no-progress, completed)
    sort_by,            // Поле для сортування (name, progress, group, department)
    sort_direction      // Напрямок сортування (asc, desc)
  } = req.query;
  ...
  try {
    ...
    // Масив параметрів для підготовленого запиту
    const params = [];
    
    // Додаємо умову пошуку за текстом, якщо вказано
    if (q && q.trim() !== '') {
      sql += ` AND CONCAT_WS(' ', 
        s.student_id, s.full_name, s.email, s.student_card_number, 
        s.department, s.phone, s.total_progress, s.diploma_id,
        g.group_name, g.specialty
      ) LIKE ?`;
      params.push(`%${q}%`);
    }
    
    // Додаємо фільтр за групою, якщо вказано
    if (group_id && group_id.trim() !== '') {
      sql += ` AND s.study_group_id = ?`;
      params.push(group_id);
    }
    
    // Додаємо фільтр за спеціальністю, якщо вказано
    if (specialty_id && specialty_id.trim() !== '') {
      sql += ` AND g.specialty = ?`;
      params.push(specialty_id);
    }
    
    // Додаємо фільтр за департаментом/кафедрою, якщо вказано
    if (department_id && department_id.trim() !== '') {
      sql += ` AND s.department = ?`;
      params.push(department_id);
    }
    
    // Додаємо фільтр за керівником, якщо вказано
    if (supervisor_id && supervisor_id.trim() !== '') {
      sql += ` AND s.supervisor_id = ?`;
      params.push(supervisor_id);
    }
    
    // Додаємо фільтр за прогресом, якщо вказано
    if (progress_filter) {
      switch (progress_filter) {
        case 'low':
          sql += ` AND (s.total_progress < 30 OR s.total_progress IS NULL)`;
          break;
        case 'medium':
          sql += ` AND s.total_progress >= 30 AND s.total_progress <= 70`;
          break;
        case 'high':
          sql += ` AND s.total_progress > 70`;
          break;
        case 'no-progress':
          sql += ` AND (s.total_progress = 0 OR s.total_progress IS NULL)`;
          break;
        case 'completed':
          sql += ` AND s.total_progress = 100`;
          break;
      }
    }
    ...
    if (sort_by) {
      ...
      switch (sort_by) {
        case 'name':
          sql += ` ORDER BY s.full_name ${direction}`;
          break;
        case 'progress':
          sql += ` ORDER BY s.total_progress ${direction}`;
          break;
        case 'group':
          sql += ` ORDER BY g.group_name ${direction}`;
          break;
        case 'department':
          sql += ` ORDER BY s.department ${direction}`;
          break;
        default:
          sql += ` ORDER BY s.student_id ASC`;
      }
    } else {
      // Якщо сортування не вказано, сортуємо за ID студента
      sql += ` ORDER BY s.student_id ASC`;
    }
...
// Отримання списку груп для фільтрації
app.get('/api/groups', async (req, res) => {
  try {
    const sql = `
      SELECT 
        study_group_id, group_name, specialty, course
      FROM study_groups
    ...
    console.error(err);
    res.status(500).json({ error: 'Помилка при отриманні списку груп' });
  }
});

// Отримання списку спеціальностей для фільтрації
...
// Отримання списку департаментів/кафедр для фільтрації
app.get('/api/departments', async (req, res) => {
  try {
    const sql = `
      SELECT DISTINCT
...
// Отримання списку наукових керівників для фільтрації
app.get('/api/supervisors', async (req, res) => {
  try {
    const sql = `
      SELECT 
        supervisor_id, full_name, email, department
      FROM supervisors
    ...
    console.error(err);
    res.status(500).json({ error: 'Помилка при отриманні списку керівників' });
  }
});

// Отримання статистики по студентам
  ...
  try {
    const sql = `
      SELECT 
        ...
        COUNT(CASE WHEN total_progress >= 30 AND total_progress <= 70 THEN 1 END) as medium_progress,
        ...
        AVG(total_progress) as average_progress
      FROM students
    `;
    const [rows] = await db.query(sql);
    res.json(rows[0]);
...
// Отримання студентів без наукового керівника
app.get('/api/students/no-supervisor', async (req, res) => {
  try {
    const sql = `
      SELECT 
        s.student_id, s.full_name, s.email, s.student_card_number, 
      ...
      FROM students s 
      LEFT JOIN study_groups g ON s.study_group_id = g.study_group_id 
      WHERE s.supervisor_id IS NULL OR s.supervisor_id = ''
...
// Отримання студентів із низьким прогресом (менше 30%)
app.get('/api/students/low-progress', async (req, res) => {
  try {
    const sql = `
      SELECT 
        s.student_id, s.full_name, s.email, s.student_card_number, 
...
// Отримання студентів, які завершили роботу (100%)
...
app.get('/api/students/:searchType/:value', async (req, res) => {
  try {
    ...
    if (searchType === 'id') {
      ...
      query = `SELECT * FROM students WHERE full_name LIKE ?`;
      value = `%${value}%`;
    } else {
    ...
    const [students] = await connection.execute(query, [value]);
    ...
    res.status(500).json({
      success: false,
      error: 'Server error while fetching student information'
    });
  }
});

// API route for fetching group information
app.get('/api/groups/:searchType/:value', async (req, res) => {
  try {
    const connection = await db;
    const { searchType, value } = req.params;
    let query = '';
    ...
    const [groups] = await connection.execute(query, [value]);
    ...
    res.status(500).json({
      success: false,
      error: 'Server error while fetching group information'
    });
  }
});

// API route for fetching supervisor information
app.get('/api/supervisors/:searchType/:value', async (req, res) => {
  try {
    const connection = await db;
    const { searchType, value } = req.params;
    let query = '';
    ...
    const [supervisors] = await connection.execute(query, [value]);
    ...
    res.status(500).json({
      success: false,
      error: 'Server error while fetching supervisor information'
    });
  }
});
