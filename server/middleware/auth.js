...
async function authenticateWebSocket(token) {
    ...
    try {
        ...
        const [accounts] = await db.query('SELECT account_id, role, is_active FROM accounts WHERE account_id = ? LIMIT 1', [decodedPayload.accountId]);
        if (accounts.length === 0 || !accounts[0].is_active) return null;
        return { accountId: accounts[0].account_id, role: accounts[0].role };
    } catch (err) {
        return null;
    }
}
...
module.exports = {
    authenticateToken, // Для HTTP
    authenticateWebSocket // Для WebSocket
};
