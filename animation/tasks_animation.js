    ...
    document.addEventListener('click', function(event) {
      if (event.target.closest('.submit-task')) {
        ...
        if (typeof window.openSubmissionModal === 'function') {
          ...
          console.error('Функцію openSubmissionModal не знайдено');
          showNotification('Не вдалося відкрити форму відправки завдання', 'error');
        }
      }
      ...
      if (event.target.closest('.edit-task')) {
        ...
        if (userRole !== 'teacher' && userRole !== 'advisor') {
          showNotification('У вас немає прав для редагування завдання', 'warning');
          return;
        }
        
        showNotification('Функція редагування завдання знаходиться в розробці', 'info');
      }
    });
  }
